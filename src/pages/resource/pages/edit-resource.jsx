...
export default class EditResource extends React.Component {
	...
	componentDidMount() {
		...
		if (this.props.location.state) {
			...
			if (resourceItem) {
				...
				// 获取文件列表
				getFileList(resourceItem.resource_content)
					.then((result) => {
	...
	render() {
		...
		const onFinish = (values) => {
			...
			// 生成创建日期
			const resource_publish_time = Moment(new Date()).format(
				'YYYY-MM-DD HH:mm:ss'
			);
			editResource(
				getUser().token,
			...
			<Card title={'编辑幼教资源'}>
				...
				>
					...
					<Item
						...
						rules={[{ required: true, message: '请选择所属素材' }]}
					>
						<Select
							style={{ width: 200 }}
							placeholder={'请选择所属领域'}
