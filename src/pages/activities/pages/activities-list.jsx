...
export default class ActivitiesList extends React.Component {
	...
	_loadData = (page_num = 1, page_size = 4) => {
		getActivitiesList(page_num, page_size)
			.then((result) => {
	...
	columns = [
		...
		{
			title: '活动封面',
			dataIndex: 'activities_img',
			render: (text) => {
				return (
					<img
						src={config.BASE_URL + text}
						alt='活动封面'
						style={{ width: 100 }}
					/>
				);
		...
		{
			title: '活动天数',
			dataIndex: 'activities_bus_day_id',
			align: 'center',
		},
		{
			title: '首页焦点',
			...
			render: (text, record) => {
				return (
					<Switch
						...
						onChange={(checked) => {
							setFocusActivities(record.id, checked ? 1 : 0).then(
								(result) => {
									if (result && result.status === 1) {
