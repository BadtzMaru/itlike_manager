	...
	componentDidMount() {
		...
		if (this.props.location.state) {
			...
			if (activitiesItem) {
				...
				activitiesItem.activities_time = Moment(
					activitiesItem.activities_time
				);
				this.activities_form_ref.current.setFieldsValue(activitiesItem);
				// 1.1 处理活动标签
				if (activitiesItem.activities_tag) {
					this.setState({
						tags: activitiesItem.activities_tag.split(','),
					});
				}
				// 1.2 处理其他的
				this.setState({
				...
				>
					...
					>
						<RichTextEditor
							...
							uploadName={'activities_img'}
							htmlContent={this.state.activities_intro}
						/>
					</Item>
					<Item
						label={'行程安排'}
					...
					>
						<RichTextEditor
							...
							uploadName={'activities_img'}
							htmlContent={this.state.activities_trip}
						/>
					</Item>
					<Item
						label={'开营日期'}
					...
					>
						<RichTextEditor
							...
							uploadName={'activities_img'}
							htmlContent={this.state.activities_day}
						/>
					</Item>
					<Item
						label={'注意事项'}
